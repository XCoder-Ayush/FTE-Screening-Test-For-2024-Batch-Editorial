//Topic : Binary Search On Answer + Sliding Window + Multiset [Hard]
...
bool f(int n,int p,int d,vi &cost,int k){
    // display k window
    // remove others
    // in k sized window i need all d sized subarray sums
    // k>d
    ...
    st.insert(dwindowSum);
    mpp[{0,d-1}]=dwindowSum;
    for(int i=d;i<k;i++){
        kwindowSum+=cost[i];
    ...
    for(int i=k;i<n;i++){
        ...
        // Remove i-k from set
        int rem=mpp[{i-k,i-k+d-1}];
        st.erase(st.find(rem));


        int maxSum=*st.begin();
        if(kwindowSum-maxSum<=p)return 1;
    }
    return 0;
}
void solve()
